...
- **карьеры** — `websocket` (рабочий), `https-lanes` (**в текущей Rust-реализации НЕ работает**), `https`, `websocket-lanes` (эксперимент);
...
- **WebSocket ломается за CF-проксированием.** Cloudflare буферизует апгрейд/долгие
  соединения, из-за чего клиент видит «вечный реконнект», хотя TCP/TLS доходят.
  Единственный надёжный режим — **DNS-only (серое облако)** для прокси-домена.
- `https-lanes` заявлен в спеке как обход CF, но **в текущей версии релея не
  реализован** (см. «Что умеет»). Не рассчитывай на него; если нужен CF-обход —
  дорабатывай ядро.
...
- DPI-блокировки (провайдеры и т.п.) душат долгие WS-соединения даже без CF —
  для устойчивости используй свежие домены и/или чаще меняй ключи.
